...
VS = "usd"
PER_PAGE = 250
MAX_PAGES = 4      # 250 * 4 = 1000
SLEEP_SIMPLE = 1.0 # small sleep between page requests

# keywords para clasificar en las 4 categorías
...
def build_graph(coins):
    G = nx.Graph()
    # Predefined exchanges, wallets, communities
    ...
    # add exchange/wallet/community nodes
    for ex in exchanges:
    ...
    for i, c in enumerate(coins):
        ...
        # add project node with attrs
        G.add_node(node, type="project", name=name, symbol=symbol, price=price, market_cap=mc, volume=vol, category=cat)
        # Connect to a selection of exchanges: probability proportional to normalized market cap
        p = min(0.95, 0.05 + mk_norm[i])  # projects with bigger mc get higher p
        for ex in exchanges:
            if np.random.rand() < p:  # probabilistic listing
                G.add_edge(node, f"ex::{ex}", weight=1.0 * (0.5 + mk_norm[i]))
        # Connect to wallets: popular projects more likely to be in multiple wallets
        wcount = 1 + int(3 * mk_norm[i])  # from 1 to 4
        ...
        # Connect to communities (Twitter for most, others probabilistic)
...
def main():
    coins = fetch_markets()
    # Small check: if coins < desired, warn
